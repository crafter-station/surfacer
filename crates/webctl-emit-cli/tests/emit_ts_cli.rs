use webctl_ir::SiteDescriptor;

/// Mirrors `examples/news-ycombinator-com.webctl.json` so a schema change fails
...
fn emits_a_program_scriptc_can_compile() {
    let out = webctl_emit_cli::emit_ts_cli(&descriptor_with(&[("list", "read", &[])]));
...
fn the_generated_cli_carries_the_whole_descriptor() {
    let out = webctl_emit_cli::emit_ts_cli(&descriptor_with(&[
        ("list", "read", &[]),
        ("show", "read", &[("id", "42")]),
    ]));
    ...
    // The header comment names webctl as the generator, so look for an actual
    // invocation: the Rust shim spawns `webctl exec`, this program must not.
    let code = out
        .lines()
        ...
        .join("\n");
    assert!(
        !code.contains("webctl"),
        "the emitted CLI must not shell out to webctl; the Rust shim does that, this one is self-contained"
    );
}

#[test]
fn params_reach_both_help_and_the_request() {
    let out = webctl_emit_cli::emit_ts_cli(&descriptor_with(&[("user", "read", &[("id", "Hunter17")])]));
...
fn quotes_in_descriptions_do_not_break_the_emitted_program() {
    ...
    let out = webctl_emit_cli::emit_ts_cli(&descriptor);
