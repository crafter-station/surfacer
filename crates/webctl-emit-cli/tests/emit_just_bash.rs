use webctl_ir::{OperationKind, SiteDescriptor};

/// Build a descriptor with one HTTP endpoint per named operation.
///
/// Shapes mirror `examples/news-ycombinator-com.webctl.json` so a schema change
...
fn emits_stable_hook_wiring_not_legacy_executor_option() {
    let out = webctl_emit_cli::emit_executor_config(&descriptor_with(&[("list", "read")]));

    assert!(
        out.contains("createWebctlExecutor"),
...
fn write_operations_are_gated_read_operations_are_not() {
    let out = webctl_emit_cli::emit_executor_config(&descriptor_with(&[
        ("list", "read"),
        ("create", "write"),
    ]));
...
fn descriptions_with_quotes_do_not_break_the_emitted_js() {
    ...
    let out = webctl_emit_cli::emit_executor_config(&descriptor);
