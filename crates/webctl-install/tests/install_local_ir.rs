use webctl_install::{install::install_site, registry, resolve::resolve_ir, resolve::ResolvedIrSource};
...
fn write_ir(dir: &std::path::Path, site: &str) -> std::path::PathBuf {
    let path = dir.join(format!("{site}.webctl.json"));
...
fn installs_an_ir_from_a_local_path() {
    ...
    let descriptor: webctl_ir::SiteDescriptor =
    ...
    let reread = webctl_ir::read_ir(&installed.ir_path).expect("re-read installed IR");
